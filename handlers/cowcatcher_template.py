...
# AANPASSING: Pad naar config.json in de settings map
# Omdat dit script wordt aangeroepen vanuit de root (via process_manager), is het pad relatief aan de root.
...
# Model path wordt ingeladen. Als dit relatief is, moet het model in de root staan of pad aangepast.
model = YOLO(model_path, task='detect')
print("Detection model successfully loaded")

print(f"Connecting to camera: {CAMERA_NAME}")

# AANPASSING: Folder for saving screenshots nu in de 'data' map
data_root = "data"
if not os.path.exists(data_root):
    os.makedirs(data_root)

save_folder = os.path.join(data_root, f"mounting_detections_{CAMERA_ID}")
...
try:
            ...
            if highest_conf >= SAVE_THRESHOLD and not collecting_screenshots and can_send_notification:
                ...
                for i in range(len(confidence_history)):
                    if confidence_history[i] >= SAVE_THRESHOLD:
                        ...
                        if SHOW_LIVE_FEED:
                            annotated = hist_frame.copy()
                            cv2.putText(annotated, f"Conf: {hist_conf:.2f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                            event_detections.append((hist_conf, annotated, hist_timestamp, hist_original_save_path, None))
                        else:
                   ...
                   inactivity_period >= INACTIVITY_STOP_TIME:
                    ...
                    if len(current_confidences) > 0:
                        ...
                        if len(current_confidences) <= 2:
                            ...
                            if max_idx > 0: selected_indices.append(max(0, max_idx - 1))
                            if max_idx < len(event_detections) - 1: selected_indices.append(max_idx + 1)
                        ...
                        if max_conf >= NOTIFY_THRESHOLD and high_conf_detections >= MIN_HIGH_CONFIDENCE_DETECTIONS:
                            ...
                            for rank, idx in enumerate(selected_indices):
                                ...
                                print(f"Telegram queued for {stage}: {conf:.2f}")
    ...
    if 'stop_reason' not in locals(): stop_reason = "Script stopped (reason unknown)"
    
    stop_message = f"⚠️ WARNING: Cowcatcher detection script stopped at {stop_time}\nReason: {stop_reason}\nFailed: {telegram_stats['failed']}"
    if SEND_STATUS_NOTIFICATIONS: _send_telegram_message_sync(stop_message)