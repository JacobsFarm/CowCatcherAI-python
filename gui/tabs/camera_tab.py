...
class CameraTab(ctk.CTkFrame):
    def __init__(self, parent, config_manager, process_manager, main_app_ref):
        ...
        ctk.CTkLabel(self.sidebar, text="CAMERA'S", font=("", 14, "bold")).grid(row=0, column=0, padx=20, pady=(20, 10))
        
        ctk.CTkButton(self.sidebar, text="+ Nieuwe Camera", fg_color=RAL_6002, command=self.add_new_camera)\
        ...
        # Tabs aanmaken
        tab_set = self.tabs.add("Instellingen")
        tab_log = self.tabs.add("Status & Logs")

        # 3. Sub-modules laden
        # We geven 'self.refresh_camera_list' mee zodat de sub-modules de lijst kunnen updaten
        self.settings_view = CameraSettingsFrame(tab_set, self.cfg, self.refresh_camera_list)
        self.settings_view.pack(fill="both", expand=True)

        self.status_view = CameraStatusFrame(tab_log, self.pm, self.refresh_camera_list)
        self.status_view.pack(fill="both", expand=True)

        # Start
        self.refresh_camera_list()
        cameras = self.cfg.get_cameras()
        if cameras: self.select_camera(cameras[0]['id'])

    def refresh_camera_list(self):
        for w in self.scroll_cams.winfo_children(): w.destroy()
        
        for cam in self.cfg.get_cameras():
            cid = cam['id']
            c_type = cam.get('type', 'cowcatcher')
            name = cam.get('name', 'Naamloos')
            ...
            # Status indicator (Play icoon/Kleur)
    ...
    def add_new_camera(self):
        ...
        self.cfg.update_camera({"id": new_id, "name": "Nieuwe Camera", "type": "cowcatcher", "enabled": True})
        self.refresh_camera_list()
        self.select_camera(new_id)

    # Proxy voor logging (vanuit main_window -> hier -> status_view)
    def append_log(self, cam_id, message):
        # We gebruiken after(0) om thread-safe te blijven
        self.master.after(0, lambda: self.status_view.append_log(cam_id, message))
    
    # Proxy voor populate (vanuit main_window on_tab_change)
